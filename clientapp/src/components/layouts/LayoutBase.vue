          ...
          <ul>
            <li v-for="menu in menus" :key="menu.name" class="mt-1">
              <a v-if="!menu.children"
                class="cursor-pointer flex items-center gap-x-3.5 py-2 px-2.5 bg-gray-100 text-sm text-gray-800 rounded-lg hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 dark:bg-neutral-700 dark:hover:bg-neutral-700 dark:focus:bg-neutral-700 dark:text-white"
                @click="selectMenu(menu.routeName, menu.name)">
...
const selectMenu = (menuName: string, breadcrumbName: string) => {
  router.push({ name: menuName });
  breadcrumb.value = breadcrumbName;
};
