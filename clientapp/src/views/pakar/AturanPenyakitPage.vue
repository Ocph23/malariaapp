...
import type { Aturan, Gejala, Penyakit } from '@/models'
...
const modalSource = ref([] as { label: string; value: number }[])
const addModal = () => {
  modalSource.value = data.gejalas
    .filter((a) => a.is_active && !data.penyakit.aturan.find((b) => b.gejala_id === a.id))
    .map((item) => ({ label: `${item.kode} - ${item.nama}`, value: item.id }))
  HSOverlay.open('#aturanModal')
}
...
const aturanSchema = z.object({
  penyakit_id: z.number().min(1),
  gejala_id: z.number().min(1),
})

const load = () => {
  const loader = loadingService.spinner('Loading data...', { fullscreen: true })
  PenyakitService.getById(id)
    .then((response) => {
      data.penyakit = response
      aturan.value = { penyakit_id: response.id, gejala_id: 0 } as Aturan
      GejalaService.get()
        .then((response) => {
...
const handleSubmit = async () => {
  const model = {
    penyakit_id: aturan.value.penyakit_id,
    gejala_id: Number(aturan.value.gejala_id),
  } as Aturan

  const result = aturanSchema.safeParse(model)
  if (result.success) {
    const loader = loadingService.spinner('Saving data...', { fullscreen: true })
    PenyakitService.postAturan(model)
      .then((response) => {
        data.penyakit.aturan.push(response)
        toast.success('Data saved successfully!')
        HSOverlay.close('#aturanModal')
        loader.remove()
      })
      .catch((error) => {
        console.log(error)
...
const deletex = async () => {
  try {
    ...
    PenyakitService.deleteAturan(deleteId.value)
      .then(() => {
        const item = data.penyakit.aturan.findIndex((item) => item.id === deleteId.value)
        if (item !== -1) data.penyakit.aturan.splice(item, 1)
        loader.remove()
