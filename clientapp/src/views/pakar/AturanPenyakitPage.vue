<template>
  <PakarLayout>
    ...
    <div id="penyakitModal"
      class="hs-overlay hidden size-full fixed top-0 start-0 z-80 overflow-x-hidden overflow-y-auto pointer-events-none"
      role="dialog" tabindex="-1" aria-labelledby="penyakitModal-label">
      <div
        ...
        <div
          ...
          <div class="flex justify-between items-center py-3 px-4 border-b border-gray-300">
            <h3 id="penyakitModal-label" class="font-semibold text-foreground">
              Tambah Penyakit
            </h3>
            <button type="button"
              ...
              aria-label="Close" data-hs-overlay="#penyakitModal">
            ...
            <div class="p-4 overflow-y-auto">
              <div class="space-y-4 p-5">
                <div class="grid gap-y-4">
                  <!-- Form Group -->
                  <div>
                    <label for="penyakitname" class="block text-sm mb-2 dark:text-white">Kode</label>
                    <div class="relative">
                      <input v-model="penyakit.kode"
                  ...
                  <div>
                    <label for="nama" class="block text-sm mb-2 dark:text-white">Nama Penyakit</label>
                    <div class="relative">
                      <input v-model="penyakit.nama"
                        class="py-2.5 sm:py-3 px-4 block w-full border-gray-200 rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600"
                  ...
                  <div>
                    ...
                    <div class="relative">
                      <input type="number" v-model="penyakit.bobot" min="0" step="0.1"
                        class="py-2.5 sm:py-3 px-4 block w-full border-gray-200 rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600"
                  ...
                  <div>
                    ...
                    <div class="relative">
                      <textarea v-model="penyakit.solusi"
                        class="py-2.5 sm:py-3 px-4 block w-full border-gray-200 rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600"
                        aria-describedby="solusi-error"></textarea>
                    </div>
...
import type { Gejala, Penyakit } from '@/models';
...
const data = reactive({ penyakit: {} as Penyakit, gejalas: [] as Gejala[] })
...
const load = () => {
  ...
  PenyakitService.getById(id).then(response => {
    data.penyakit = response;
    loader.remove();
  }).catch(error => {
    console.log(error);
    loader.remove();
...
const handleSubmit = async () => {
  ...
  if (result.success) {
    ...
    if (penyakit.id > 0) {
      PenyakitService.put(penyakit.id, penyakit).then(() => {
        const row = data.gejalas.find(item => item.id === penyakit.id);
        if (row) {
      ...
      PenyakitService.post(penyakit as Penyakit).then(response => {
...
const deletex = async () => {
  try {
    ...
    PenyakitService.delete(deleteId.value).then(() => {
